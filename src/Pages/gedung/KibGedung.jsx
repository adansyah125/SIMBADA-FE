...
import { getKibGedung, deleteKibGedung, importExcel } from "../../services/KibGedungService";
...
function KibGedung() {
  ...
  const confirmDelete = async () => {
    ...
    try {
      await deleteKibGedung(deleteId);
      ...
      <div className="mx-auto max-w-7xl px-4">
          ...
          <table className="w-full text-left text-xs">
            <thead>
              <tr className="border-b border-gray-200 bg-gray-50 font-semibold text-gray-600 uppercase">
                ...
                <th className="px-3 py-3">NIBAR</th>
                <th className="px-3 py-3">No Register</th>
                <th className="px-3 py-3">Spesifikasi</th>
                <th className="px-3 py-3">Spesifikasi Lainnya</th>
                <th className="px-3 py-3">Jumlah Lantai</th>
                <th className="px-3 py-3">Lokasi</th>
                <th className="px-3 py-3">Titik Koordinat</th>
                <th className="px-3 py-3">Status Kepemilikan Tanah</th>
                ...
                <th className="px-3 py-3">Cara Perolehan</th>
                <th className="px-3 py-3">Tgl Perolehan</th>
                <th className="px-3 py-3">Status Penggunaan</th>
            ...
            <tbody className="divide-y divide-gray-100">
              {loading ? (
                <tr>
                  <td colSpan={20} className="py-12 text-center text-gray-400">Memuat data...</td>
                </tr>
              ) : data.length === 0 ? (
                <tr>
                  <td colSpan={20} className="py-12 text-center text-gray-400 italic">~~ Data kosong ~~</td>
                </tr>
              ) : (
                data.map((item, index) => (
                  <tr key={item.id} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-3 py-3 text-gray-600">{item.jumlah_lantai || "-"}</td>
                    <td className="px-3 py-3 text-gray-600">{item.lokasi || "-"}</td>
                    <td className="px-3 py-3 text-gray-600">{item.titik_koordinat || "-"}</td>
                    <td className="px-3 py-3 text-gray-600">{item.status_kepemilikan_tanah || "-"}</td>
                    <td className="px-3 py-3 text-gray-600">{item.jumlah || "-"}</td>
                    <td className="px-3 py-3 text-gray-600">{item.satuan || "-"}</td>
                    ...
                    <td className="px-3 py-3 text-center">
                      <div className="flex items-center justify-center gap-2">
                        <Link
                          to={`/kib/gedung/edit/${item.id}/edit`}
              ...
              <div key={item.id} className="rounded-lg border border-gray-200 bg-white overflow-hidden">
                ...
                <div className="p-4 space-y-4">
                  ...
                  <div className="grid grid-cols-2 gap-3 text-xs">
                    ...
                    <div>
                      <p className="text-gray-400">Jumlah Lantai</p>
                      <p className="font-medium text-gray-800">{item.jumlah_lantai || "-"}</p>
                    </div>
                    <div>
                    ...
                    <div className="col-span-2">
                      <p className="text-gray-400">Titik Koordinat</p>
                      <p className="font-medium text-gray-800">{item.titik_koordinat || "-"}</p>
                    </div>
                    <div>
                      <p className="text-gray-400">Status Kepemilikan Tanah</p>
                      <p className="font-medium text-gray-800">{item.status_kepemilikan_tanah || "-"}</p>
                    </div>
                    <div>
        ...
        <div className="mt-6 flex items-center justify-between">
          <p className="text-sm text-gray-500">
            Halaman {currentPage} dari {lastPage} (Total {total} data Aset Gedung)
          </p>
