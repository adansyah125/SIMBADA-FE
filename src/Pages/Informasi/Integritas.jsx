        ...
        <div className="flex-1">
          ...
          {loading ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {data.map((integritas) => (
                <div key={integritas.id} className="bg-white p-5 rounded-2xl border border-gray-100 shadow-sm hover:shadow-md transition-shadow">
                  <div className="flex justify-between items-start mb-3">
                    
                    <span className="ml-auto text-xs text-gray-400 italic">{formatTanggal(integritas.tanggal)}</span>
                  </div>
                  
                  <h3 className="font-bold text-gray-800 text-lg mb-1">{integritas.nama}</h3>
                  <p className="text-xs text-gray-500 mb-4 font-medium">NIP: {integritas.nip || '-'}</p>
                  ...
                  <div className="grid grid-cols-2 gap-2 pt-4 border-t border-gray-50">
                    <div className="text-center p-2 bg-gray-50 rounded-lg">
                      ...
                      <p className="text-xs font-semibold text-gray-700 truncate">{integritas.jabatan}</p>
                    </div>
                    <div className="text-center p-2 bg-green-50 rounded-lg">
                      <p className="text-[10px] text-green-500 uppercase font-bold">Total Unit</p>
                      <p className="text-xs font-bold text-green-700">{integritas.jumlah} Unit</p>
                    </div>
                  </div>
                  <div className="mt-3 text-[10px] text-gray-400 flex items-center gap-1">
                    ...
                    Aset Mesin: {integritas.mesin?.nama_barang || '-'} ({integritas.mesin?.merk || '-'})
                  </div>
                  {/* BUTTON UNDUH */}
                  <div className="flex gap-2 mt-4">
                <button 
                  onClick={() => downloadPdf(integritas.id)}
                ...
                <button 
                  onClick={() => handleDelete(integritas.id)}
