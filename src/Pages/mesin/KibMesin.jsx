      ...
      <div className="mx-auto max-w-7xl px-4">
          ...
          <table className="w-full text-left text-xs">
            <thead>
              ...
              <tr className="border-b border-gray-200 bg-gray-50 font-semibold text-gray-600 uppercase">
                ...
                <th rowSpan={2} className="px-3 py-3">NIBAR</th>
                <th rowSpan={2} className="px-3 py-3">No Register</th>
                <th rowSpan={2} className="px-3 py-3">Spesifikasi</th>
                <th rowSpan={2} className="px-3 py-3">Spesifikasi Lainnya</th>
                <th rowSpan={2} className="px-3 py-3">Merk</th>
                <th rowSpan={2} className="px-3 py-3">Lokasi</th>
                <th colSpan={3} className="px-3 py-3 text-center border-x border-gray-200">Kendaraan Dinas</th>
                <th rowSpan={2} className="px-3 py-3">Jumlah</th>
                <th rowSpan={2} className="px-3 py-3">Satuan</th>
                <th rowSpan={2} className="px-3 py-3">Harga Satuan</th>
                <th rowSpan={2} className="px-3 py-3">Nilai Perolehan</th>
                <th rowSpan={2} className="px-3 py-3">Cara Perolehan</th>
                <th rowSpan={2} className="px-3 py-3">Tgl Perolehan</th>
                <th rowSpan={2} className="px-3 py-3">Status Penggunaan</th>
                <th rowSpan={2} className="px-3 py-3">Keterangan</th>
                <th rowSpan={2} className="px-3 py-3 text-center w-20">Aksi</th>
              </tr>
              {/* Row 2 — sub header Kendaraan Dinas */}
              <tr className="border-b border-gray-200 bg-gray-50 font-semibold text-gray-500 uppercase">
                <th className="px-3 py-2 border-x border-gray-200">No Polisi</th>
                <th className="px-3 py-2 border-r border-gray-200">No Rangka</th>
                <th className="px-3 py-2">No BPKB</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-gray-100">
              {loading ? (
                <tr>
                  <td colSpan={21} className="py-12 text-center text-gray-400">Memuat data...</td>
                </tr>
              ) : data.length === 0 ? (
                <tr>
                  <td colSpan={21} className="py-12 text-center text-gray-400 italic">~~ Data kosong ~~</td>
                </tr>
              ) : (
                data.map((item, index) => (
                  <tr key={item.id} className="hover:bg-gray-50 transition-colors">
                    <td className="px-3 py-3 text-center text-gray-400">{index + 1 + (currentPage - 1) * 10}</td>
                    ...
                    <td className="px-3 py-3 text-gray-600">{item.nibar || "-"}</td>
                    <td className="px-3 py-3 text-gray-600">{item.no_register || "-"}</td>
                    <td className="px-3 py-3 text-gray-600">{item.spesifikasi_nama_barang || "-"}</td>
                    <td className="px-3 py-3 text-gray-600">{item.spesifikasi_lainnya || "-"}</td>
                    <td className="px-3 py-3 text-gray-600">{item.merk || "-"}</td>
                    <td className="px-3 py-3 text-gray-600">{item.lokasi || "-"}</td>
                    {/* Kendaraan Dinas */}
                    <td className="px-3 py-3 text-gray-600 border-x border-gray-100">{item.no_polisi || "-"}</td>
                    <td className="px-3 py-3 text-gray-600 border-r border-gray-100">{item.no_rangka || "-"}</td>
                    <td className="px-3 py-3 text-gray-600">{item.no_bpkb || "-"}</td>
                    {/* Nilai */}
                    <td className="px-3 py-3 text-gray-700">{item.jumlah ?? "0"}</td>
                    <td className="px-3 py-3 text-gray-700">{item.satuan || "-"}</td>
                    ...
                    <td className="px-3 py-3 text-gray-600">{item.cara_perolehan || "-"}</td>
                    <td className="px-3 py-3 text-gray-600">{item.tanggal_perolehan ? formatTanggal(item.tanggal_perolehan) : "-"}</td>
                    <td className="px-3 py-3 text-gray-600">{item.status_penggunaan || "-"}</td>
                    <td className="px-3 py-3 text-gray-500 max-w-[150px] truncate">{item.keterangan || "-"}</td>
                    {/* Aksi */}
                    <td className="px-3 py-3 text-center">
                      <div className="flex items-center justify-center gap-2">
                        <Link
                          to={`/kib/mesin/edit/${item.id}/edit`}
              ...
              <div key={item.id} className="rounded-lg border border-gray-200 bg-white overflow-hidden">
                ...
                <div className="flex items-center justify-between border-b border-gray-100 bg-gray-50 px-4 py-3">
                  <div className="flex items-center gap-2">
                    <span className="text-xs text-gray-400">#{index + 1 + (currentPage - 1) * 10}</span>
                ...
                <div className="p-4 space-y-4">
                  ...
                  <div className="grid grid-cols-2 gap-3 text-xs">
                    <div>
                      <p className="text-gray-400">NIBAR</p>
                      <p className="font-medium text-gray-800">{item.nibar || "-"}</p>
                    </div>
                    <div>
                      <p className="text-gray-400">No Register</p>
                      <p className="font-medium text-gray-800">{item.no_register || "-"}</p>
                    </div>
                    <div>
                      <p className="text-gray-400">Spesifikasi</p>
                      <p className="font-medium text-gray-800">{item.spesifikasi_nama_barang || "-"}</p>
                    </div>
                    <div>
                      <p className="text-gray-400">Spesifikasi Lainnya</p>
                      <p className="font-medium text-gray-800">{item.spesifikasi_lainnya || "-"}</p>
                    </div>
                    <div>
                      <p className="text-gray-400">Merk</p>
                      <p className="font-medium text-gray-800">{item.merk || "-"}</p>
                    </div>
                    <div>
                    ...
                    <div>
                      <p className="text-gray-400">No Polisi</p>
                      <p className="font-medium text-gray-800">{item.no_polisi || "-"}</p>
                    </div>
                    <div>
                      <p className="text-gray-400">No Rangka</p>
                      <p className="font-medium text-gray-800">{item.no_rangka || "-"}</p>
                    </div>
                    <div>
                      <p className="text-gray-400">No BPKB</p>
                      <p className="font-medium text-gray-800">{item.no_bpkb || "-"}</p>
                    </div>
                    <div>
                      <p className="text-gray-400">Jumlah</p>
                      <p className="font-medium text-gray-800">{item.jumlah ?? "0"}</p>
                    </div>
                    <div>
                      <p className="text-gray-400">Satuan</p>
                      <p className="font-medium text-gray-800">{item.satuan || "-"}</p>
                    </div>
                    <div>
                      <p className="text-gray-400">Cara Perolehan</p>
                      <p className="font-medium text-gray-800">{item.cara_perolehan || "-"}</p>
                    </div>
                    <div>
                    ...
                    <div>
                      <p className="text-gray-400">Status Penggunaan</p>
                      <p className="font-medium text-gray-800">{item.status_penggunaan || "-"}</p>
                    </div>
        ...
        <div className="mt-6 flex items-center justify-between">
          <p className="text-sm text-gray-500">
            Halaman {currentPage} dari {lastPage} (Total {total} data Aset Mesin)
          </p>
